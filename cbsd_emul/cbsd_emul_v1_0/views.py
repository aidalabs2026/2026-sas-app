...
@view_config(route_name='msglogtableview', renderer='./templates/msgTableView.jinja2')
def msglogtableview_view(request):
    device_id = request.matchdict.get('device_id')
    sub_id = request.matchdict.get('sub_id')
...
@view_config(route_name='api/cbsd_delete', renderer='json')
def cbsd_delete(request):
    # JSON 데이터를 받아옴
    cbsd_id = request.params.get('cbsd_id', 0)

    cbsdApi.cbsd_delete(cbsd_id)
...
def relinquishment(request):
    try:
        ...
        if responseCode != 0:
            ...
            # Create a PeriodicThread that runs every 5 seconds
            heartbeatManager.stop_heartbeat(payload["grantId"])
