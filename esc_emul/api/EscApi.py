...
class EscApi:
    ...
    def registed_sensor_list(self):
        try:
            return self.EscDao.esc_registed_list()
        except pymysql.MySQLError as e:
            print(f"Error checking sensor existence: {e}")
            return None
    ...
    def sensing_result_report(self, esc, reqdata):
        ...
        try:
            ...
            self.EscDao.esc_last_report_dt_update(esc)

            self.SeDao.update_channels_status()
    ...
    def sensing_result_process(self, esc_sensor_id, sensing_result):
        ...
        try:
            ...
            pre_incumbentUserActivation = self.EscDao.esc_ch_get_status(esc_sensor_id, lowFrequency, highFrequency)
            ...
            if incumbentUserActivation:  # esc센서가 incumbent user 를 감지했을 때,
                ...
                for grant in grant_list:
                    ...
                    # 4. TD_CBSD_SE ESC 무브리스트의 채널 별 가용 파워 테이블을 업데이트 한다.


            else:  # esc 센서가 incumbent user 를 해제했을 때
                #grant_list = self.CbsdDao.grant_list_by_freq(lowFrequency, highFrequency, "GRANTED", 1)
                grant_list = self.CbsdDao.get_move_list_by_e_dpa(esc_sensor_id, lowFrequency, highFrequency, "GRANTED", 1)
