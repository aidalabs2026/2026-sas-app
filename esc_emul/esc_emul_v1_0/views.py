...
from api.EscEmulApi import EscEmulApi
...
from esc_emul_v1_0.SensingManager import SensingManager
...
escEmulApi = EscEmulApi(db_config)
...
def regist(request):
    try:
        ...
        settings_manager.get_setting("sasurl")

        response = requests.post(f'{settings_manager.get_setting("sasurl")}esc_regist', json=payload)
        ...
        escEmulApi.channel_insert(payload['escSensorId'])
...
def deregist(request):
    try:
        ...
        response = requests.post(f"{settings_manager.get_setting('sasurl')}esc_deregist", json=payload)
...
def sensing(request):
    try:
        ...
        response = requests.post(f"{settings_manager.get_setting('sasurl')}esc_sensing", json=payload)
...
def incumbent_change(request):
    try:
        ...
        escEmulApi.esc_ch_update_status(sensor_id, lowFreq, highFreq, incumbentUserActivation)
...
def esc_list(request):
    esc_list = escEmulApi.sensor_list()
...
def esc_sensor_delete(request):
    ...
    escEmulApi.sensor_sensor_delete(sensor_id)
...
def esc_ch_list(request):
    ...
    esc_ch_list = escEmulApi.sensor_channel_list(sensor_id)
