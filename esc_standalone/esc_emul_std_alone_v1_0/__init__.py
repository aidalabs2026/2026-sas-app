...
def hello(request):
    ...
    for msg in ws:
        if msg.type == web.WSMsgType.TEXT:
            ...
            pass#client_manager.remove_client(ws)
    return Response('Hello world!')
...
def main(global_config, **settings):
    ...
    config.add_route('api/esc_sensor_delete', '/api/esc_sensor_delete')
    ...
    config.add_route('api/incumbent_change', '/api/incumbent_change')
    config.add_route('api/release', '/api/release')
    config.add_route('api/sensing_on', '/api/sensing_on')
    config.add_route('api/sensing_check', '/api/sensing_check')
    config.add_route('api/sensing_report', '/api/sensing_report')
    config.add_route('api/save_setting', '/api/save_setting')
    config.add_route('api/load_setting', '/api/load_setting')
    config.add_route('api/sas_status', '/api/sas_status')
    config.add_route('api/health', '/api/health')
    config.add_route('ws', '/ws')
    config.include('pyramid_jinja2')
    # 정적 파일 서빙 경로 설정
    config.add_static_view(name='static', path='esc_emul_std_alone_v1_0:static', cache_max_age=3600)
