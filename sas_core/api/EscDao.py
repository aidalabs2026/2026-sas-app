...
class EscDao:
    ...
    def esc_ch_exists(self, sensor_id):
        try:
            ...
            with self.connection.cursor() as cursor:
                query = """SELECT COUNT(*) as count FROM TD_ESC_CHANNELS WHERE SENSOR_ID = %s"""
                cursor.execute(query, (sensor_id,))
                result = cursor.fetchone()
    ...
    def esc_channels_insert(self, esc):
        ...
        try:
            with self.connection.cursor() as cursor:
                query = """INSERT INTO TD_ESC_CHANNELS (SENSOR_ID, CH_NO, LOW_FREQ, HIGH_FREQ) 
                                                   VALUES (%s, %s, %s, %s)"""
    ...
    def sensor_channel_list(self, sensor_id):
        try:
            with self.connection.cursor() as cursor:
                query = """SELECT * FROM TD_ESC_CHANNELS WHERE SENSOR_ID=%s ORDER BY CH_NO ASC"""
                cursor.execute(query,
                               (sensor_id,))
                result = cursor.fetchall()
                return result
    ...
    def esc_ch_update_status(self, sensor_id, low_freq, high_freq, status):
        try:
            with self.connection.cursor() as cursor:
                query = """UPDATE TD_ESC_CHANNELS SET INCUMBENT_USER=%s WHERE SENSOR_ID = %s AND LOW_FREQ=%s AND HIGH_FREQ=%s"""
    ...
    def esc_ch_get_status(self, sensor_id, low_freq, high_freq):
        try:
            with self.connection.cursor() as cursor:
                query = """SELECT INCUMBENT_USER FROM TD_ESC_CHANNELS WHERE SENSOR_ID = %s AND LOW_FREQ=%s AND HIGH_FREQ=%s"""
