...
class SnDao:
    ...
    def sn_insert(self, fcc_id, mfsd_id):
        if not self.connection.open:
            self.connect()
    ...
    def update_channels_to_nonavail(self, fcc_id, mfsd_id, occupied_channels):
        ...
        sql = f"UPDATE TD_CBSD_SN SET {set_clause} WHERE FCC_ID = %s AND MFSD_ID = %s"
    ...
    def update_channels_to_avail(self, fcc_id, mfsd_id, ch, status=-999):
        """
        conn : MySQL connection 객체
        record_id : TD_CBSD_SF 테이블의 ID
        occupied_channels : [1,2,3] 처럼 점유된 채널 번호 리스트
